...
class Callback:
    ...
    def load(self, trainer):
        if not os.path.exists(self.out_dir) or not os.listdir(self.out_dir):
            raise FileNotFoundError(f"No saved models found in {self.out_dir}")
        ...
        snapshot = torch.load(model_file, map_location='cpu')
        trainer.model.load_state_dict(snapshot['model_state'])  # Load model weights
        trainer.model = trainer.model.to(trainer.device)  # Move model to the specified device
...
#         trainer.model.load_state_dict(snapshot['model_state']).to(trainer.device)
    ...
    def sample(self, trainer, save_dir):
        num_images = 500  # Total images to generate in one pass
        ...
        for s in range(self.log_max_steps):
            N = 2 ** s  # Step size
            x0 = torch.randn(shape, device=trainer.device)  # Generate random noise
            x1 = sample_fun(x0, N) * 0.5 + 0.5  # Generate images from the model

            # Create a directory for the current step
            step_dir = os.path.join(save_dir, f"step{str(N).zfill(4)}")
            os.makedirs(step_dir, exist_ok=True)

            # Save all 500 images in the step directory
            for i in range(num_images):
                single_image_path = os.path.join(
                    step_dir, f"img_{i:03d}.png"  # Image name with zero-padded index
                )
